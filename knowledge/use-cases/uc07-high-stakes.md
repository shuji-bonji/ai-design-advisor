...
- パラメータ: Temperature 0.0–0.3。Effort は計画ターンだけ
- 知識: RAG + Grounding。根拠が無ければ答えない
- 実行系: 段 2–4。品質ゲートを早める。最終判定をモデルに渡さない
- Fine-tune: しない
- 権限: permission 反復。承認は操作本文
- 測り方: 跡、根拠、人の采用。NIST / AISI 表は合格線にしない
...
| D6 | 品質ゲートを早める（段 4）。初手 Multi-agent にしない | when-to-agentize / quality-gate | 確定 |
...
- **代替**: 読み取り専用なら段 2 で止め、承認 UI だけ厚くする。
- **禁止**: 自己レビューで完結、NIST / AISI 表を採点、知識を FT 。

### ベンダーに渡す文

- やること: 引用、跡、操作本文の承認、主体の契約明記、品質ゲート
- やらないこと: 自動決裁、ガイドライン全文の転記、黒箱オーケストレータ

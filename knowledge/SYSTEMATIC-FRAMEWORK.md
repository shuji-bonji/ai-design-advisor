...
> 充足状況の正本は [`canonical/_index.md`](./canonical/_index.md)。下表の「状態」列は次元の定義用。
...
- **安定コーパス**（原則凍結）… Zenn、understanding-llm、ai-agent-architecture。すでに canonical へ折込み済み

スキルの根拠は canonical のみ。安定コーパスを4系統として毎回重み付けない。

各次元のノートと残課題は `canonical/` 配下を読む。
...
| 継続 | 一次情報の追加・更新・削除 | Effort / モデル世代 / OWASP 年次 |

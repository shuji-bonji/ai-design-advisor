...
- **承認する人は一人か、役割で分けるか**（起案と承認を同じ会話・同じ人にしないか）
...
**許可判定は LLM に任せない**（OWASP LLM03:2026）。画面に出す前にアプリ側の allowlist で止める。

制御フローはコードが持つ（12-Factor 8）。特に **ツール選択と実行の間** で一時停止できること。ここが HITL の挿入点になる。

**HITL は「止める」だけでなく「誰が止めるか」を決める。** 書き込みや高リスクの判定は起案者と承認者を分ける。一人しかいないなら、その制約を追加確認に残す。
